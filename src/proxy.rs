...
async fn tunnel_bidirectional(client: &mut TcpStream, remote: &mut TcpStream) -> anyhow::Result<()> {
    #[cfg(target_os = "linux")]
    {
        if let Ok(_) = splice_bidirectional(client, remote).await {
            return Ok(());
        }
    }
...
#[cfg(target_os = "linux")]
async fn splice_bidirectional(client: &mut TcpStream, remote: &mut TcpStream) -> anyhow::Result<()> {
    let client_fd = client.as_raw_fd();
    let remote_fd = remote.as_raw_fd();

    let client_to_remote = tokio::task::spawn_blocking(move || {
        splice_stream(client_fd, remote_fd)
    });

    let remote_to_client = tokio::task::spawn_blocking(move || {
        splice_stream(remote_fd, client_fd)
    });

    tokio::select! {
        res1 = client_to_remote => { res1??; }
        res2 = remote_to_client => { res2??; }
    }
...
#[cfg(target_os = "linux")]
fn splice_stream(src_fd: i32, dst_fd: i32) -> Result<(), std::io::Error> {
    let mut pipe = [0i32; 2];
    unsafe {
    ...
    let result = (|| {
        loop {
            ...
            while written < bytes_in as usize {
                let bytes_out = unsafe {
                    libc::splice(
                        pipe_rd,
                        std::ptr::null_mut(),
                        dst_fd,
                        std::ptr::null_mut(),
                        (bytes_in as usize) - written,
                        libc::SPLICE_F_MOVE,
                    )
                };
                if bytes_out <= 0 {
